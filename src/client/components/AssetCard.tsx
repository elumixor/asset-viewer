...
import { AssetCard2D } from "./AssetCard2D";
  ...
  return (
    <div className="bg-[#152233] border border-[#223547] rounded-xl p-2.5 flex flex-col gap-2 relative overflow-hidden">
      <div className="bg-[#111] border border-[#223547] rounded-lg flex items-center justify-center aspect-square relative overflow-hidden">
        <div className="absolute top-1.5 left-1.5 bg-[#4ec9ff] text-[#052030] text-[10px] px-1.5 py-[3px] rounded font-bold uppercase tracking-[0.5px]">{type}</div>
        {isLoading ? (
          ...
          <AssetCard3D gltf={asset} onMeta={setMeta} />
        ) : asset ? (
          <AssetCard2D texture={asset} onMeta={setMeta} />
        ) : null}
      </div>
      <div className="text-[0.85rem] font-semibold text-[#9cd6ff] whitespace-nowrap overflow-hidden text-ellipsis">{filename}</div>
